...
pub fn parse_message_sections(message_bytes: &[u8]) -> Result<Grib1Sections> {
    ...
    let pds_length = read_u24(&message_bytes[8..11]) as usize;
    let pds = section(1, 8, pds_length);
    ...
    let grid = if product.has_grid_definition {
        let gds_length = read_u24(&message_bytes[cursor..cursor + 3]) as usize;
        let section_ref = section(2, cursor, gds_length);
        cursor += gds_length;
        Some(section_ref)
    } else {
        None
    ...
    let bitmap = if product.has_bitmap {
        let bms_length = read_u24(&message_bytes[cursor..cursor + 3]) as usize;
        let section_ref = section(3, cursor, bms_length);
        cursor += bms_length;
        Some(section_ref)
    } else {
        None
    ...
    let bds_length = read_u24(&message_bytes[cursor..cursor + 3]) as usize;
    let data = section(4, cursor, bds_length);
